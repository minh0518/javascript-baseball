...
class App {
	...
	//생성자로 기본 세팅 (컴퓨터의 정답 받기 , 게임 시작 메세지 등)
  constructor() {
  ...
  getComputerRandomNumber() {
	...
	//게임 로직. 이 단위로 게임이 돌아가는 것이며 
	//테스트를 통과하기 위한 메인 로직이다
  play() {
    MissionUtils.Console.readLine(
      ...
      (userInputNumber) => {
        this.userInputNumber = userInputNumber
				...
				//숫자를 입력받으면 예외처리 , 스트라이크 볼 판별 , 정답 출력
        this.isError()  
        this.count()
        this.printMessage()
        ...
        if (this.countStrike < 3) {
          this.countStrike = 0
          this.countBall = 0
          this.play() //재귀로 게임 재시작(3스트라이크 아니니까)
  ...
  count() {
    ...
    const computerStringArray = this.computerNumberArray.map((num) =>
      num.toString(),
    )
